...
public interface DeliveryMapper {
	@Select("SELECT * FROM wine_delivery WHERE userId=#{userId} AND state!=9 ORDER BY state DESC, wdno DESC")
	public List<DeliveryVO> myDeliveryList(String userId);
	
	@Insert("INSERT INTO wine_delivery values((SELECT NVL(MAX(wdno)+1, 1) FROM wine_delivery), #{name}, #{userId}, #{post}, #{addr1}, #{addr2}, #{msg}, #{state}, #{addrEng})")
	public void deliveryInsert(DeliveryVO vo);
	
	@Update("UPDATE wine_delivery SET state=0 WHERE userId=#{userId} AND state!=9")
	...
	@Update("UPDATE wine_delivery SET state=9 WHERE wdno=#{wdno}")
